...
def check_slug(name):
    ...
    The database is the source of record, but `wl import` rebuilds it from
    work_log.md when it has to, and that rescue path needs every slug heading to be
    readable. A slug with whitespace renders a heading the parser cannot see, so
    entries under it would be unrecoverable rather than lost at some later point a
    reader might notice.
    """
    if not re.fullmatch(r"\S+", name):
        sys.exit(f"error: slug {name!r} must be one word with no whitespace, or "
                 "entries under it are unrecoverable if `wl import` ever has to "
...
def parse_markdown(text):
    """Entries only, header and pre-day lines silently ignored. Use scan_markdown when
    the dropped lines matter."""
    return scan_markdown(text)[0]


# No line here may start with "- ": the guard reads this file back before every write
# and counts such a line as an entry it cannot parse, so one in the header refuses every
# write from the second command onwards, and the refusal's own advice (delete
# work_log.md, run `wl render`) writes the header back. export_md asserts against it.
HEADER_BLOCK = (
    "# Work Log\n\n"
    ...
    "`wl rm`. This file is read back on every write: a hand-edit that adds or\n"
    "rewrites an entry is refused rather than absorbed, as is a line that cannot be\n"
    "read back at all."
)
...
def warn_if_legacy_log_ignored(legacy_root=None):
    ...
    Keyed on the new log holding no entries, not on work_log.db existing: connect()
    creates that file (via ensure_root()) on any command, including a read, so an
    exists() check went permanently silent after the first command ever run, while the
    real log sat untouched at the legacy path. Checking emptiness means opening a
    connection, so the cheap legacy.exists() check runs first, to avoid creating the
    new root on a genuine first run that has nothing to warn about.
    """
    if os.environ.get("WORKLOG_ROOT"):
        return
    ...
    conn = connect()
    try:
        has_entries = conn.execute("SELECT 1 FROM entries LIMIT 1").fetchone()
    finally:
        conn.close()
    if not has_entries:
...
def connect():
    ...
    migrate(conn)
    # ponytail: slugs live only in the database, never in work_log.md, so a from-scratch
    # `wl import` reseeds just 'general' and any other registered slugs are gone.
...
def export_md(conn):
    ...
    rendered, unreadable = scan_markdown(text)
    # A rendered line the parser skips is this file's bug and never the reader's, so it
    # raises rather than exiting. It has to be caught here because the guard counts such
    # a line as an entry at stake: one emitted by the header or the renderer refuses
    # every later write, and the refusal's advice writes it back.
    if unreadable:
        raise AssertionError(f"render produced {len(unreadable)} line(s) `wl import` "
                             f"cannot read back: {unreadable[:3]}")
    survived = set(map(_roundtrip_key, rendered))
